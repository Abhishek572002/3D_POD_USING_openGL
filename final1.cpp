...
void setLighting()
{
    ...
    GLfloat light_position[] = {100.0, 100.0, 0, 1};
...
float angle = 0, xx = 0, yy = -15, zz = -150;
...
void init(void)
{
    ...
    // The modelview matrix in OpenGL combines the model and view matrices to represent the position, orientation, and scale of objects being rendered.
...
void resize(int width, int height)
{
    ...
    glLoadIdentity();
    // set up perspective projection with specified parameters
    gluPerspective(90.0, width / (height | 1), 1.0, 630.0);
    // set matrix mode to modelview
    glMatrixMode(GL_MODELVIEW);
    // load identity matrix
    glLoadIdentity();
    // apply translation to the modelview matrix
    glTranslatef(0.0, -15, -150);
}
...
int main(int argc, char **argv)
{
    ...
    // call to initialize the OpenGL graphics library
    init();
    // sets a timer to call a function (timer) after a specified time (0ms) with no arguments (0).
